...
// Get Job
func (s *jobServer) GetJob(g *gin.Context) {
	...
	item, err := s.Job.Get(namespace, name)
	if err != nil {
...
// Subscribe Job
func (s *jobServer) ListJob(g *gin.Context) {
	list, err := s.Job.List(g.Param("namespace"), "")
	if err != nil {
...
func (s *jobServer) ApplyJob(g *gin.Context) {
	namespace := g.Param("namespace")
	...
	newUnstructuredExtend, isUpdate, err := s.Job.Apply(namespace, name, &service.UnstructuredExtend{Unstructured: _unstructured})
	if err != nil {
...
// Delete Job
func (s *jobServer) DeleteJob(g *gin.Context) {
	namespace := g.Param("namespace")
	name := g.Param("name")
	if namespace == "" || name == "" {
		common.RequestParametersError(g, fmt.Errorf("params not obtain namespace=%s name=%s", namespace, name))
	...
	err := s.Job.Delete(namespace, name)
	if err != nil {
		common.InternalServerError(g, err, err)
		return
	}
