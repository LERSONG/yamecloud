...
func (g *CronJob) Get(namespace, name string) (*service.UnstructuredExtend, error) {
	item, err := g.Interface.Get(namespace, k8s.CronJobs, name)
	if err != nil {
		return nil, err
	}
	return item, nil
...
func (g *CronJob) List(namespace string, selector string) (*service.UnstructuredListExtend, error) {
	list, err := g.Interface.List(namespace, k8s.CronJobs, selector)
	if err != nil {
		return nil, err
	}
	return list, nil
...
func (g *CronJob) Apply(namespace, name string, unstructuredExtend *service.UnstructuredExtend) (*service.UnstructuredExtend, bool, error) {
	item, isUpdate, err := g.Interface.Apply(namespace, k8s.CronJobs, name, unstructuredExtend)
	if err != nil {
...
func (g *CronJob) Delete(namespace, name string) error {
	err := g.Interface.Delete(namespace, k8s.CronJobs, name)
	if err != nil {
		return err
	}
	return nil
