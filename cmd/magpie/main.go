...
import (
	...
	"github.com/yametech/yamecloud/pkg/action/api/magpie"
...
func main() {
	...
	magpie.CreateSharedSessionManager(config.Clientset, config.Config)
	_datasource := datasource.NewInterface(config)
	apiServer := api.NewServer(service.NewService(_datasource))
	apiServer.SetExtends(magpie.NewMagpieServer(serviceName, apiServer))
